...
class Spark {
  ...
  onFrame(event) {
    if (this.sparking)
    {
      ...
      if (this.hist.length >= this.sparkLength || this.position > this.followPath.length)
      {
      ...
      if (this.position <= this.followPath.length)
        this.hist.push(this.pos);

      // Add new tail head
      if (this.hist.length > 1 && this.position < this.followPath.length)
      {
        this.nextPath = this.pathCallback(this.options, this.position, this.hist, this.nextPath);

        this.paths.push(this.nextPath);
        this.nextPath = undefined;
      }

      this.paths.forEach((p, ix) => this.pathRedraw(p, ix / this.sparkLength));
    }
  }
  ...
  constructor(options) {
    ...
    this.sparkLength = options.sparkLength || 60;   // Number of shapes for this spark
